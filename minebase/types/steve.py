...
# we see a violation.
HEX_COLOR_RE = re.compile(r"^#(?:[0-9a-fA-F]{1}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$")
...
class SkinPieceTintColorsData(MinecraftDataModel):
    ...
    colors: list[Annotated[str, Field(pattern=HEX_COLOR_RE)]]
    piece_type: str
...
class SteveData(MinecraftDataModel):
    ...
    skin_color: str = Field(pattern=HEX_COLOR_RE)
    skin_id: str
